    ...
    <div>
      ...
      {toastNudge && !dismissed && (
        <div className="fixed bottom-7 right-7 z-50 bg-[#FFFCF7] rounded-xl p-5 shadow-md w-72">
          <button className="absolute top-3 right-3 text-[#4A4A4A] text-sm bg-transparent border-none cursor-pointer" onClick={() => dismissNudge(toastNudge._id)}>✕</button>
      ...
      {dismissed && (
        <div className="fixed bottom-7 right-7 z-50 bg-[#F2EFE9] text-[#4A4A4A] text-[0.78rem] px-4 py-2 rounded-full shadow-sm">Nudge dismissed ●</div>
      )}
    </div>
  );
}